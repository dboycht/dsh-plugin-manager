 ...
 *
 * Adds a "备注" (Notes) tab to Settings → Plugins and talks to the Host half
 * through the same-origin JSON API at `/_dsh/plugin-manager` (see lib/index.js).
 * Plain React.createElement only — no JSX, no build step.
 */
...
function PluginManagerTab() {
  const [state, setState] = useState({ loading: true, error: null, deployment: [], dynamic: [], notes: {}, notesFile: null })
  ...
  const load = () => {
    api('/list').then((res) => {
      ...
      setState({ loading: false, error: String(e && e.message ? e.message : e), deployment: [], dynamic: [], notes: {}, notesFile: null })
    })
  }
  ...
  for (const d of state.deployment) {
    rows.push({ kind: '部署', key: d.id, name: d.name, sub: '', statusKey: d.enabled ? (d.phase === 'active' ? 'active' : 'enabled') : 'disabled', note: state.notes[d.id], agentId: null, currentPackageId: null, manageable: false })
    seen[d.id] = true
  }
  for (const d of state.dynamic) {
    ...
    rows.push({
      ...
      agentId: d.agentId, currentPackageId: d.currentPackageId, manageable: !!d.manageable,
    })
    seen[d.pluginId] = true
  }
  for (const key of Object.keys(state.notes)) {
    if (!seen[key]) rows.push({ kind: '自定义', key, name: '', sub: '', statusKey: 'custom', note: state.notes[key], agentId: null, currentPackageId: null, manageable: false })
  }
