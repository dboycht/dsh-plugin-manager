...
export function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    name: 'plugin_manager',
    description: '插件管理：列出 DeepSeek Harness 的全部插件（部署插件 + 动态 Cordis 插件）及已保存的备注；读取/写入/删除备注；导出/导入全部备注（备份与恢复）；启动/停止动态插件；enable/disable 启停插件管理（停用后所有写操作被拒绝，重新启用即恢复）。备注持久化在工作区根目录的 .dsh-plugin-notes.json。',
    parameters: {
      action: { type: 'string', enum: ['list', 'get', 'set', 'remove', 'export', 'import', 'start', 'stop', 'enable', 'disable'], required: true, description: '操作：list 列出全部；get 读取某键备注；set 写入/更新备注；remove 删除备注；export 导出全部备注 JSON；import 导入备注（note 参数放 JSON）；start 启动动态插件；stop 停止动态插件；enable/disable 启用/停用插件管理' },
